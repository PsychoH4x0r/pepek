...
module.exports.welcome = async (iswel, isleft, Kyami, anu) => {
  try {
    const metadata = await Kyami.groupMetadata(anu.id);
    const participants = anu.participants;
    const groupName = metadata.subject;
    const groupDesc = metadata.desc;
    for (let num of participants) {
      try {
        pp_user = await Kyami.profilePictureUrl(num, 'image');
      } catch {
      ...
      if (anu.action === 'add' && (iswel || setting.auto_welcomeMsg)) {
        if (isSetWelcome(anu.id, set_welcome_db)) {
          ...
          Kyami.sendMessage(anu.id, { text: `${full_pesan}` });
        } else {
          Kyami.sendMessage(anu.id, { text: `*Ahlan WA Sahlan kepada ${num.split("@")[0]} di ${groupName}* *_Salam Ukhuwah, Semoga Istiqomah_* 🤗 jangan lupa aktif mengikuti *kegiatan* dan *Kajian* yang sudah ada 📚 
ꔛ⌑ *Budayakan* ⌑ꔛ
...
\`\`\`Semoga Bermanfaat\`\`\`` });
        ...
        if (isSetLeft(anu.id, set_left_db)) {
          ...
          const full_pesan = replace_pesan.replace(/@group/gi, groupName).replace(/@desc/gi, groupDesc);
          Kyami.sendMessage(anu.id, { image: { url: pp_user }, mentions: [num], caption: `${full_pesan}` });
        } else {
          Kyami.sendMessage(anu.id, { text: `*Selamat Jalan* kepada ${num.split("@")[0]} dari grup tercinta kami  𓈒 ִ۫  @${groupName} *kita do'akan semoga ilmu dari grup ini bermanfaat bagi beliau* 🤲🏻
